...
const convertUserSolutionToCardUserSolution = async (userSolution: UserSolution, challenge: Challenge): Promise<CardUserSolution> => {
...
export const UsersSolutionsCards = async ({ challenge }: { challenge: Challenge }) => {
    ...
    const cardUserSolutions = await Promise.all(usersSolutions.map((userSolution) => convertUserSolutionToCardUserSolution(userSolution, validatedChallenge)))
