...
function TypographyP({ children, className = '', id }: TypographyProps) {
  return <p id={id} className={`leading-7 [&:not(:first-child)]:mt-6 ${className} text-muted-foreground`}>{children}</p>
}
...
const INITIAL_COMPONENTS: Partial<Components> = {
  ...
  strong: function StrongComponent({ children, ...props }) {
    return <TypographyBold {...props} className='text-gray-200'>{children}</TypographyBold>
  },
